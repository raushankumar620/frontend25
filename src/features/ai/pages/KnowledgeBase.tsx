    ...
    <PageContainer>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4 mb-6 sm:mb-8">
        <div className="bg-white border border-[#E2EAE6] p-3.5 sm:p-4 rounded-2xl shadow-xs">
          <div className="text-xs font-semibold text-[#5F7069]">Indexed Documents</div>
          <div className="text-xl sm:text-2xl font-black text-[#14201C] mt-1">{docs.length}</div>
          <span className="text-[10px] sm:text-[11px] text-[#05A222] font-semibold mt-0.5 inline-block">Active in RAG</span>
        </div>
        <div className="bg-white border border-[#E2EAE6] p-3.5 sm:p-4 rounded-2xl shadow-xs">
          <div className="text-xs font-semibold text-[#5F7069]">Total Vector Chunks</div>
          <div className="text-xl sm:text-2xl font-black text-[#14201C] mt-1">{totalChunksCount}</div>
          <span className="text-[10px] sm:text-[11px] text-[#8A9993] font-medium mt-0.5 inline-block">Embedded segments</span>
        </div>
        <div className="bg-white border border-[#E2EAE6] p-3.5 sm:p-4 rounded-2xl shadow-xs">
          <div className="text-xs font-semibold text-[#5F7069]">Vector Embedding Model</div>
          <div className="text-xs sm:text-sm font-bold font-mono text-[#14201C] mt-2">text-embedding-3-small</div>
          <span className="text-[10px] sm:text-[11px] text-[#006736] font-semibold mt-0.5 inline-block">1536-dim Cosine</span>
        </div>
        <div className="bg-white border border-[#E2EAE6] p-3.5 sm:p-4 rounded-2xl shadow-xs">
          <div className="text-xs font-semibold text-[#5F7069]">Grounding Status</div>
          ...
          <span className="text-[10px] sm:text-[11px] text-[#8A9993] font-medium mt-0.5 inline-block">Auto-injected into prompt</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm sm:text-base font-bold text-[#14201C] flex items-center gap-2">
          ...
          {loading ? (
            ...
            <div className="p-8 text-center bg-white rounded-2xl border border-[#E2EAE6] space-y-3">
              <div className="w-12 h-12 mx-auto rounded-2xl bg-[#E9F9EE] text-[#006736] border border-[#C4EBD0] flex items-center justify-center">
                <BookOpen className="w-6 h-6 text-[#05A222]" />
              </div>
              <h4 className="text-sm font-bold text-[#14201C]">No Documents in Knowledge Base</h4>
        ...
        <div className="lg:col-span-5 space-y-5 sm:space-y-6">
          {/* Web Sync Card */}
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[#E2EAE6] space-y-4 shadow-xs">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-[#E9F9EE] text-[#006736] border border-[#C4EBD0] flex items-center justify-center font-bold shrink-0">
                <Globe className="w-4 h-4" />
              </div>
              <div>
                <h4 className="text-xs font-bold text-[#14201C]">Sync from Website URL</h4>
                <p className="text-[11px] text-[#5F7069]">Crawl and vectorize web pages or help centers.</p>
              </div>
            </div>
          ...
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[#E2EAE6] space-y-4 shadow-xs">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h4 className="text-xs font-bold text-[#14201C]">RAG Semantic Search Playground</h4>
                <p className="text-[11px] text-[#5F7069]">Test vector retrieval and inspect similarity scores.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSemanticSearch} className="space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-xl border border-[#E2EAE6] bg-[#F6FAF8] text-[#14201C] placeholder-[#8A9993] text-xs px-3 py-2 focus:outline-none focus:border-[#05A222] focus:bg-white transition-colors"
                />
                <Button
                  size="sm"
                  type="submit"
                  ...
                  className="bg-[#05A222] hover:bg-[#006736] text-white font-bold px-3 rounded-xl shrink-0"
                >
            ...
            {hasSearched && (
              <div className="space-y-2.5 pt-2 border-t border-[#E2EAE6]">
                <div className="text-[11px] font-bold text-[#5F7069]">
                  Retrieved Chunks ({searchResults.length})
                </div>

                {searchResults.length === 0 ? (
                  <div className="p-4 bg-[#F6FAF8] rounded-xl text-center text-xs text-[#8A9993] border border-[#E2EAE6]">
                  ...
                  searchResults.map((r, idx) => (
                    <div
                      key={idx}
                      className="p-3 bg-[#F6FAF8] rounded-xl border border-[#E2EAE6] space-y-1.5 text-xs"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-bold text-[#006736] text-[11px] truncate">
                          {r.documentTitle} (Chunk #{r.chunkIndex + 1})
                        </span>
                        <span className="text-[10px] font-mono font-bold bg-[#E9F9EE] text-[#006736] border border-[#C4EBD0] px-1.5 py-0.5 rounded shrink-0">
                          {(r.similarityScore * 100).toFixed(1)}% match
                        </span>
                      </div>
                      <p className="text-[11px] text-[#5F7069] line-clamp-3 leading-relaxed">
                        {r.text}
                      </p>
                    </div>
                  ))
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-[#E2EAE6] rounded-2xl max-w-xl w-full p-5 sm:p-6 space-y-5 shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-[#E2EAE6] pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-bold text-[#14201C]">Index New Knowledge Document</h3>
                  <p className="text-xs text-[#5F7069]">Auto-chunked and embedded for RAG retrieval.</p>
                </div>
              </div>
              <button
                onClick={() => setShowAddModal(false)}
                className="p-1 rounded-lg text-[#5F7069] hover:text-[#14201C] cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-xs font-bold flex flex-col items-center gap-1 transition-all cursor-pointer ${
                        newType === t.id
                          ...
                          : 'border-[#E2EAE6] bg-[#F6FAF8] text-[#5F7069] hover:bg-white hover:border-[#C4EBD0]'
                      }`}
                    >
